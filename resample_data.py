#!/usr/bin/env python3
"""
Resample all audio files in a directory tree to a target sample rate.
...
def resample_file(
    ...
    try:
        ...
        # ffmpeg encodes by output extension
        cmd = [
            "ffmpeg",
            "-y",
            "-i", str(src_path),
...
def _resample_task(args: Tuple[str, Optional[str], int]) -> Tuple[str, bool, str]:
    """Module-level wrapper for ProcessPoolExecutor (picklable)."""
    src, dst, target_sr = args
    return resample_file(src, target_sr, dst)
...
def main(
    ...
    logger.info(
        f"Found {len(audio_files)} audio files to resample to {target_sample_rate}Hz "
    ...
    task_args = [(src, dst, target_sample_rate) for src, dst in tasks]
